...
public class MusicReader {
	 ...
	 */
	public int BSfindIndex(RhythmEvent[] events, int time) {
		int start = 0;
	 ...
	 */
	public RhythmEvent[] easyVersion(RhythmEvent[] events) {
	...
	public RhythmEvent[] populationDouble(RhythmEvent[] events) {
		int numToAdd = events.length / 4;
		RhythmEvent[] ret = new RhythmEvent[events.length + numToAdd];
		for (int i = 0; i < (events.length - 2) && numToAdd > 0; i++) {
			int rand = (int) (Math.random() * 12);
			if (rand == 7) { // 1 in 12 chance
				// Add 3 notes
				ret[i] = events[i];
				ret[i + 1] = events[i].copy();
				ret[i + 1].setCircleIndex(4 - events[i].getCircleIndex());
				ret[i + 2] = events[i + 1];
				ret[i + 3] = events[i + 1].copy();
				ret[i + 3].setCircleIndex(4 - events[i + 1].getCircleIndex());
				ret[i + 4] = events[i + 2];
				ret[i + 5] = events[i + 2].copy();
				ret[i + 5].setCircleIndex(4 - events[i + 2].getCircleIndex());
				numToAdd -= 3;
			} else {
				ret[i] = events[i];
			}
	 ...
	 */
	public static RhythmEvent[] createYouBelongWithMeTS() {
		MusicReader mr = new MusicReader();
		RhythmEvent[] melodyEvents = mr.extractMidiInformation(new File("tswift_belong.mid"), 1, 130);
		RhythmEvent[] drumEvents = mr.extractMidiInformation(new File("tswift_belong.mid"), 14, 130);
	...
	public static void main(String[] args) {
		MusicReader mr = new MusicReader();
		RhythmEvent[] songEvents = MusicReader.createYouBelongWithMeTS();
		mr.outputInformation(songEvents);
		try {
			mr.playMusic(songEvents, (float) 0.0, 120, 130);
